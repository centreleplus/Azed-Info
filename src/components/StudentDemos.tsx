...
import { 
  ...
  Video, Flame, Tag, Layers, Maximize2, X, RefreshCw, Film, ExternalLink,
  Check, Filter, ChevronRight, PlayCircle
} from 'lucide-react';
...
export const StudentDemos: React.FC<StudentDemosProps> = ({ 
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-6 space-y-7 select-none text-left font-sans">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/90 dark:border-slate-800 shadow-sm p-4 sm:p-6 space-y-4 transition-colors">
        <div className="flex items-center justify-between pb-1 border-b border-slate-100 dark:border-slate-800">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 text-xs font-bold text-purple-600 dark:text-purple-400 hover:text-purple-700 bg-purple-50 dark:bg-purple-950/50 hover:bg-purple-100 px-3 py-1.5 rounded-xl transition cursor-pointer"
          >
      ...
      {/* 6. MODAL PLEIN ÉCRAN POUR LECTURE CINÉMA */}
      <AnimatePresence>
        {modalVideo && (
          <div className="fixed inset-0 z-50 bg-black/85 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 animate-fadeIn">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-2xl max-w-4xl w-full border border-slate-200 dark:border-slate-800 flex flex-col max-h-[92vh]"
            >
              {/* Header Modal */}
              ...
              <div className="aspect-video bg-black w-full shrink-0">
                <iframe
                  src={getEmbedVideoUrl(modalVideo.videoUrl || modalVideo.youtubeId)}
              ...
              {/* Footer Modal avec description */}
              <div className="p-4 sm:p-5 bg-white dark:bg-slate-900 overflow-y-auto space-y-3">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div className="flex items-center gap-2">
                    <span className={`px-2.5 py-0.5 text-[11px] font-black rounded-lg border ${getCategoryBadgeColor(modalVideo.category)}`}>
                      {modalVideo.category}
                    </span>
                  ...
                  >
                    <span>Définir dans le lecteur principal</span>
                    <ArrowRight className="w-3.5 h-3.5" />
                  </button>
                </div>

                <p className="text-xs sm:text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
                  {modalVideo.description || "Aucune description renseignée pour cette vidéo."}
                </p>
              </div>
            </motion.div>
          </div>
      ...
      >
        ...
        <div className="p-3 bg-slate-50 dark:bg-slate-850/80 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-2">
          ...
          >
            ...
            <span>{isSelected ? 'Lecture active' : 'Lire'}</span>
          </button>

          <button
            type="button"
            onClick={() => setModalVideo(video)}
            className="p-1.5 text-slate-500 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-lg transition cursor-pointer flex items-center gap-1 text-[11px] font-bold"
            title="Ouvrir en mode Plein Écran / Modal"
          >
            <Maximize2 className="w-3.5 h-3.5" />
